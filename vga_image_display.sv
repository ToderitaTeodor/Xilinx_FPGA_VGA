module vga_image_display #(
    ...
    input  logic [11:0] h_counter_i   ,  // Current horizontal pixel
    input  logic [11:0] v_counter_i   ,  // Current vertical pixel

    input  logic        video_active_i,  // Active display region
...
assign dx = (h_counter_i >= CENTER_X) ? (h_counter_i - CENTER_X) : (CENTER_X - h_counter_i);
assign dy = (v_counter_i >= CENTER_Y) ? (v_counter_i - CENTER_Y) : (CENTER_Y - v_counter_i);
...
always_comb begin
    ...
    if (video_active_i && (radius_i != 0) && (distance_sq <= radius_sq)) begin

        // Inner ring
        if ((distance_sq < radius_sq / 5) && (heat_level_i >= 4)) begin
            red_next   = 4'hD;
            green_next = 4'h2;
            blue_next  = 4'h1;
        end
        ...
        // Ring 2
        else if ((distance_sq < 2 * radius_sq / 5) && (heat_level_i >= 3)) begin
            red_next   = 4'hD;
            green_next = 4'h4;
            blue_next  = 4'h1;
        end
        ...
        // Ring 3
        else if ((distance_sq < 3 * radius_sq / 5) && (heat_level_i >= 2)) begin
            red_next   = 4'hC;
            green_next = 4'h6;
            blue_next  = 4'h2;
        end
        ...
        // Ring 4
        else if ((distance_sq < 4 * radius_sq / 5) && (heat_level_i >= 1)) begin
            red_next   = 4'hC;
            green_next = 4'h8;
            blue_next  = 4'h2;
        end
